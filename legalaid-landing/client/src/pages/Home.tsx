      ...
      <main id="top">
        ...
        <section id="method" className="relative bg-[#f1eee6] py-24 sm:py-32">
          <div className="container grid gap-16 lg:grid-cols-[0.8fr_1.2fr] lg:gap-28">
            <div><span className="eyebrow text-[#3158ff]">The premise</span><h2 className="font-display mt-7 max-w-117.5 text-5xl font-bold leading-[0.95] tracking-[-0.07em] sm:text-7xl">Legal review should leave a trail.</h2><p className="mt-8 max-w-97.5 text-base leading-7 text-[#626860]">Most AI review tools give you an answer. LegalAid gives you the route taken to reach it — and marks the places where the route breaks.</p></div>
        ...
        <section id="signals" className="bg-[#101412] py-24 text-[#f1eee6] sm:py-32"><div className="container"><div className="flex flex-col justify-between gap-8 border-b border-white/15 pb-10 sm:flex-row sm:items-end"><div><span className="eyebrow text-[#d7ff52]">What it surfaces</span><h2 className="font-display mt-6 max-w-167.5 text-5xl font-bold leading-[0.92] tracking-[-0.07em] sm:text-7xl">Risk is not a number. It is a set of receipts.</h2></div><p className="max-w-75 text-sm leading-6 text-[#8f978e]">Every specialist agent is constrained to retrieved document context. No evidence, no confident claim.</p></div><div className="mt-12 grid gap-px bg-white/15 md:grid-cols-2 lg:grid-cols-4">{[{icon: FileSearch, n:"01", title:"Exploit paths", desc:"Clauses that create leverage, ambiguity, or one-sided exposure."},{icon: ScanText, n:"02", title:"Draft weakness", desc:"Definitions, cross-references, and obligations that do not hold."},{icon: ShieldCheck, n:"03", title:"Compliance gaps", desc:"Missing protections and obligations that leave a process open."},{icon: Fingerprint, n:"04", title:"Citation integrity", desc:"Claims that cannot survive a return to the source text."}].map((item) => <article key={item.n} className="group min-h-67.5 bg-[#151a17] p-7 transition-colors hover:bg-[#1c241e]"><div className="flex items-start justify-between"><item.icon className="h-7 w-7 text-[#d7ff52]" strokeWidth={1.4} /><span className="font-mono text-xs text-[#626b61]">{item.n}</span></div><h3 className="font-display mt-20 text-2xl font-semibold tracking-[-0.04em]">{item.title}</h3><p className="mt-3 text-sm leading-6 text-[#8f978e]">{item.desc}</p></article>)}</div></div></section>

        <section id="pipeline" className="bg-[#e6e2d8] py-24 sm:py-32"><div className="container grid gap-16 lg:grid-cols-[0.7fr_1.3fr] lg:gap-28"><div><span className="eyebrow text-[#3158ff]">The pipeline</span><h2 className="font-display mt-6 text-5xl font-bold leading-[0.92] tracking-[-0.07em] sm:text-7xl">Seven passes.<br /><span className="text-[#3158ff]">One readable report.</span></h2><p className="mt-8 max-w-90 text-base leading-7 text-[#626860]">A document moves through a structured review loop built to expose uncertainty, not hide it.</p><button onClick={() => goTo("top")} className="mt-10 flex items-center gap-3 font-mono text-xs uppercase tracking-[0.12em] text-[#3158ff] hover:underline">Back to the top <ChevronRight className="h-4 w-4" /></button></div><div className="relative space-y-0 border-l border-[#a9afa7] pl-7 sm:pl-12">{[{n:"01",title:"Upload + validate",desc:"Accept the source and establish a trustworthy document boundary."},{n:"02",title:"Extract + chunk",desc:"Parse pages, clauses, parties, and tokens with stable metadata."},{n:"03",title:"Retrieve + dispatch",desc:"Search the corpus and route context to parallel specialist agents."},{n:"04",title:"Verify + converge",desc:"Reject unsupported findings, merge the surviving signals, and score the risk."},{n:"05",title:"Report + inspect",desc:"Return a readable report with source-grounded evidence at every turn."}].map((step, i) => <div key={step.n} className="relative border-b border-[#c5c0b6] py-7 first:pt-0 last:border-0"><span className="absolute -left-10.5 top-8 grid h-4 w-4 place-items-center rounded-full bg-[#3158ff] ring-4 ring-[#e6e2d8] sm:-left-14.5"><span className="h-1.5 w-1.5 rounded-full bg-[#d7ff52]" /></span><div className="flex gap-6"><span className="font-mono text-xs text-[#3158ff]">{step.n}</span><div><h3 className="font-display text-2xl font-semibold tracking-[-0.04em]">{step.title}</h3><p className="mt-2 max-w-112.5 text-sm leading-6 text-[#626860]">{step.desc}</p></div></div></div>)}</div></div></section>

        <section className="bg-[#101412] py-24 text-[#f1eee6] sm:py-32"><div className="container grid items-end gap-12 lg:grid-cols-[1.3fr_0.7fr]"><div><div className="mb-7 flex items-center gap-3"><Sparkles className="h-5 w-5 text-[#d7ff52]" /><span className="eyebrow text-[#d7ff52]">Guardrails by default</span></div><h2 className="font-display max-w-205 text-5xl font-bold leading-[0.9] tracking-[-0.08em] sm:text-[6.5rem]">When evidence runs out, the system says so.</h2></div><p className="max-w-75 text-sm leading-6 text-[#8f978e]">Agents must cite retrieved text, keep document content out of system prompts, and return <span className="font-mono text-[#d7ff52]">VERIFICATION_UNAVAILABLE</span> when a claim cannot be grounded.</p></div></section>

        <section id="access" className="relative overflow-hidden bg-[#d7ff52] py-20 sm:py-28"><div className="absolute right-0 top-0 h-full w-1/3 opacity-20" style={{ backgroundImage: "url('/manus-storage/legalaid-evidence_11fc7503.png')", backgroundPosition: "center", backgroundSize: "cover", mixBlendMode: "multiply" }} /><div className="container relative flex flex-col justify-between gap-10 sm:flex-row sm:items-end"><div><span className="eyebrow text-[#3158ff]">Open the case file</span><h2 className="font-display mt-5 max-w-195 text-5xl font-bold leading-[0.9] tracking-[-0.08em] text-[#101412] sm:text-7xl">Make the document defend itself.</h2></div><Button onClick={() => openAuth("/signup")} className="h-14 shrink-0 rounded-none bg-[#101412] px-7 font-mono text-xs uppercase tracking-[0.12em] text-[#d7ff52] hover:bg-[#263026]">Request access <ArrowUpRight className="ml-3 h-4 w-4" /></Button></div></section>
      </main>
