  ...
  return (
    <main className="min-h-screen bg-[#101412] text-[#f1eee6]">
      <div className="relative min-h-screen overflow-hidden lg:grid lg:grid-cols-[minmax(420px,0.82fr)_1.18fr]">
        ...
        <section className="relative z-10 flex min-h-screen flex-col border-r border-white/10 bg-[#101412]/95 px-6 py-6 sm:px-10 lg:px-16">
          ...
          <div className="mx-auto flex w-full max-w-[420px] flex-1 flex-col justify-center py-16">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {!isSignup && <div className="flex justify-end"><button type="button" onClick={() => toast("Password reset flow coming next.")} className="font-mono text-[10px] uppercase tracking-widest text-[#d7ff52] hover:underline">Forgot password?</button></div>}
        ...
        <section className="relative hidden min-h-screen overflow-hidden lg:block"><div className="absolute inset-0 bg-[#101412]" /><div className="absolute inset-0 bg-cover bg-center opacity-95" style={{ backgroundImage: "url('/manus-storage/legalaid-lady-justice_c94d69fe.png')" }} /><div className="absolute inset-0 bg-[linear-gradient(90deg,#101412ee_0%,transparent_30%,#10141233_100%)]" /><div className="absolute inset-y-0 left-12 border-l border-[#d7ff52]/35" /><div className="absolute bottom-12 left-20 max-w-90"><p className="font-display text-3xl font-semibold leading-[0.98] tracking-[-0.05em] text-[#f1eee6]">The strongest opinion is the one that can show its work.</p></div></section>
      </div>
    </main>
  );
}
